 ...
 */
const DYN_ARG_REGEX =
  /(?<arg1>(?:str\s*:\s*\`(?<arg1_val_str>[^`]*)\`)|(?:key\s*:\s*(?:{(?<arg1_val_key>.*?)}))|(?:fn\s*:\s*(?:{(?<arg1_val_fn>.*?)}))|(?:num\s*:\s*(?<arg1_val_num>[0-9]+(\.[0-9]+)?))|(?:bool\s*:\s*(?<arg1_val_bool>true|false|1|0)))(?:\s*,\s*(?<arg2>(?:str\s*:\s*\`(?<arg2_val_str>[^`]*)\`)|(?:key\s*:\s*(?:{(?<arg2_val_key>.*?)}))|(?:fn\s*:\s*(?:{(?<arg2_val_fn>.*?)}))|(?:num\s*:\s*(?<arg2_val_num>[0-9]+(\.[0-9]+)?))|(?:bool\s*:\s*(?<arg2_val_bool>true|false|1|0))))?/s;
...
export const getFunctionParameters = <T extends Record<string, unknown>>(
  str: string,
